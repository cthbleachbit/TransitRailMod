...
public class ClosedPlatformDoorItem extends Item {
	...
	private Block door = ModBlocks.closed_platform_door_block;
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (facing != EnumFacing.UP) {
			...
			if (!worldIn.getBlockState(pos).getBlock().isNormalCube(worldIn.getBlockState(pos), worldIn, pos)){
				...
				if (canPlace) {
					IBlockState panelState = door.getDefaultState().withProperty(ClosedPlatformPanelBlock.FACING, playerIn.getHorizontalFacing());
